...
// ***********************************************************************
// <copyright file="CustomWebViewRenderer.designer.cs" company="XLabs Team">
...
namespace FeatureDemo.Forms.Controls
{
    ...
    public partial class CustomWebViewRenderer
    {
        ...
        private const string NativeFunction = "function Native(action, data){window.webkit.messageHandlers.native.postMessage(JSON.stringify({ 'action': action, 'data': data }));}";
#elif __ANDROID__
        private const string NativeFuncCall = "Xamarin.call";
        private const string NativeFunction = "function Native(action, data){Xamarin.call(JSON.stringify({ 'action': action, 'data': data }));}";
#endif
        ...
        protected override void OnElementPropertyChanged(object sender, System.ComponentModel.PropertyChangedEventArgs e)
        {
            ...
            else if (e.PropertyName == CustomWebView.SourceProperty.PropertyName)
            {
                LoadSource();
            }
            else if (e.PropertyName == CustomWebView.CleanupProperty.PropertyName)
            {
                HandleCleanup ();
            }
        }
        ...
        private string GetFuncScript()
        {
            ...
            builder.Append("(JSON.stringify({ 'action': action, 'data': data, 'callback': callbackIdx }));}");
        ...
        private void Unbind(CustomWebView oldElement)
        {
            if (oldElement != null)
            {
